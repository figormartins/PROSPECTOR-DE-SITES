...
- [ ] **Domínio limpo e humano.** Se o `projeto` do config colidiu com outro na Vercel e ganhou um sufixo aleatório (tipo `estudio-paginas-a1b2c3.vercel.app`), PARE antes de enviar qualquer proposta: link assim parece golpe e mata a confiança que a capa constrói. Oriente o usuário a ajustar o nome do `projeto` (Configurações do dashboard) pra um sem sufixo, ou anexar um domínio próprio ao projeto direto no painel da Vercel e preencher o campo `dominio`. Proposta só sai com domínio apresentável.
