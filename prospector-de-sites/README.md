...
1. `/setup` — roda uma vez: assinatura, nichos padrão, conexão com a Vercel (com teste de publicação).
...
5. `/publicar [cliente|todos]` — sobe na Vercel em `dominio.com/clientes/[slug]/`, gera a página-capa de apresentação (antes/depois personalizado, `proposta.html`) e só conclui com HTTPS validado.
...
O pacote inclui `manual.html` — o manual completo do usuário, copiado pra pasta no `/setup` e **atualizado a cada versão do plugin**. A publicação na Vercel é automática: CLI logada uma vez (`vercel login`, fora do chat) e o `/publicar` roda o deploy sozinho, dentro do próprio chat — sem instalador, sem login repetido.
...
- Conta na Vercel com a CLI instalada e logada (`vercel login`)

## Onde ficam os dados

Tudo na pasta conectada: `prospector-config.json` (preferências, sem credenciais — a Vercel CLI cuida da autenticação), `leads.md` (pipeline) e `sites/[slug]/` (páginas criadas).
